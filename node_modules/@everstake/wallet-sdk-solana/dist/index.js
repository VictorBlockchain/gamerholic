  ...
  constructor(network = "mainnet-beta" /* Mainnet */, rpc = null) {
    ...
    switch (network) {
      case "mainnet-beta" /* Mainnet */:
        rpc = rpc || (0, import_web32.mainnet)("https://api.mainnet-beta.solana.com");
        this.validator = MAINNET_VALIDATOR_ADDRESS;
        break;
      case "devnet" /* Devnet */:
        rpc = rpc || (0, import_web32.devnet)("https://api.devnet.solana.com");
    ...
    try {
      this.connection = (0, import_web32.createSolanaRpc)(rpc);
    } catch (error) {
  ...
  async claim(sender, params) {
    try {
      ...
      const deactivatedStakeAccounts = delegations.result.filter((acc) => {
        const isDeactivated = !isLockupInForce(acc.data, epoch, BigInt(tm)) && stakeAccountState(acc.data, epoch) === StakeState.Deactivated;
        if (isDeactivated) {
