...
export const cookieStorage = (config) => {
    if (config?.sessionLength) {
        console.warn("The cookieStorage sessionLength option is deprecated and will be ignored.");
    }
    return {
...
export function cookieToInitialState(config, cookie) {
    ...
    alchemyClientState.config.sessionConfig = {
        ...alchemyClientState.config.sessionConfig,
        expirationTimeMs: config.store.getInitialState().config.sessionConfig?.expirationTimeMs ??
            alchemyClientState.config.sessionConfig?.expirationTimeMs,
    };
    const wagmiClientState = wagmiCookieToInitialState(config._internal.wagmiConfig, decodeURIComponent(cookie));
    return {
