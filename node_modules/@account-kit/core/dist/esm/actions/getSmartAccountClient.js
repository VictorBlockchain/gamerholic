...
import { accountLoupeActions, lightAccountClientActions, multiOwnerLightAccountClientActions, multiOwnerPluginActions, pluginManagerActions, } from "@account-kit/smart-contracts";
 ...
 */
export function getSmartAccountClient(params, config) {
    ...
    const clientState = config.store.getState().smartAccountClients[connection.chain.id]?.[type];
    ...
    if (!account ||
        ...
        const loadingState = {
            client: undefined,
            address: status === "RECONNECTING" || status === "READY"
                ? account.address
                : undefined,
    ...
    const newState = (() => {
        switch (account.source) {
            ...
            case "ModularAccountV2":
                const is7702 = isModularV2AccountParams(params) &&
                ...
                return {
                    client: createAlchemySmartAccountClient({
                        ...
                        ...(is7702
                            ? {
                                gasEstimator: default7702GasEstimator(clientParams.gasEstimator),
                                signUserOperation: default7702UserOpSigner(clientParams.signUserOperation),
                            }
                            : {}),
                        ...clientParams,
                    }),
...
function getSmartAccountClientState({ config, chainId, type, }) {
    return config.store.getState().smartAccountClients[chainId][type];
}
function setSmartAccountClientState({ config, newState, type, connection, }) {
...
function assertNeverAccountType(_accountType) {
    throw new Error("Unsupported account type");
}
//# sourceMappingURL=getSmartAccountClient.js.map