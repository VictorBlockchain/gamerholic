 ...
 */ export function watchSmartAccountClient(params, config) {
    return (onChange) => {
        const accounts_ = config.store.getState().accounts;
        if (!accounts_) {
            throw new ClientOnlyPropertyError("account");
        }
        return config.store.subscribe(({ signerStatus, accounts, chain }) => ({
            signerStatus,
            ...
            equalityFn(a, b) {
                return (a.signerStatus === b.signerStatus &&
                    a.account === b.account &&
                    a.chain === b.chain);
            },
        });
...
//# sourceMappingURL=watchSmartAccountClient.js.map