...
export const createAccountKitStore = (params) => {
    const { connections, storage = typeof window !== "undefined" ? localStorage : undefined, ssr, } = params;
    ...
    const store = createStore(subscribeWithSelector(storage
        ? persist(() => createInitialStoreState(params), {
            ...
            storage: createJSONStorage(() => storage, {
                ...
                reviver: (key, value) => {
                    if (key === "chain") {
                        return connections.find((c) => c.chain.id === value.id)?.chain;
                    }
            ...
            merge: (persisted, current) => {
                ...
                if (!connections.every((c) => persistedState.connections.has(c.chain.id) &&
                ...
                return {
                    ...
                    ...current,
                    ...persistedState,
                    smartAccountClients: createEmptySmartAccountClientState(connections.map((c) => c.chain)),
                    connections: connectionsMap,
                    bundlerClient: createAlchemyPublicRpcClient({
                        chain: persistedState.chain,
                        transport: alchemy(persistedState.connections.get(persistedState.chain.id)
                            .transport),
                    }),
                };
            },
...
const createInitialStoreState = (params) => {
    ...
    const baseState = {
        ...
        signerStatus: convertSignerStatusToState(AlchemySignerStatus.INITIALIZING, undefined),
 ...
 */
export const convertSignerStatusToState = (alchemySignerStatus, error) => ({
    ...
    store.subscribe(({ signer }) => signer, (signer) => {
        ...
        signer.on("statusChanged", (status) => {
            store.setState((state) => ({
                signerStatus: convertSignerStatusToState(status, state.signerStatus.error),
            }));
        });
        ...
        signer.on("disconnected", () => {
            const chains = [...store.getState().connections.values()].map((c) => c.chain);
            store.setState({
                user: undefined,
        ...
        signer.on("errorChanged", (error) => store.setState((state) => ({
            signerStatus: convertSignerStatusToState(state.signerStatus.status, error),
        })));
...
export const createDefaultAccountState = (chains) => {
    return chains.reduce((acc, chain) => {
        acc[chain.id] = {
            ...
            MultiOwnerLightAccount: defaultAccountState(),
            ModularAccountV2: defaultAccountState(),
        };
        return acc;
    }, {});
};
export const createEmptySmartAccountClientState = (chains) => {
    return chains.reduce((acc, chain) => {
        acc[chain.id] = {};
        return acc;
    }, {});
