...
import { convertSignerStatusToState, createDefaultAccountState, defaultAccountState, } from "./store/store.js";
/**
...
const hydrateAccountState = (accountConfigs, shouldReconnectAccounts, config) => {
    const chains = Array.from(config.store.getState().connections.entries()).map(([, cnx]) => cnx.chain);
    ...
    return Object.entries(accountConfigs).reduce((acc, [chainKey, config]) => {
        ...
        acc[chainId] = {
            LightAccount: shouldReconnect && config.LightAccount?.accountAddress
            ...
            MultiOwnerModularAccount: shouldReconnect && config.MultiOwnerModularAccount?.accountAddress
                ...
                : defaultAccountState(),
            MultiOwnerLightAccount: shouldReconnect && config.MultiOwnerLightAccount?.accountAddress
                ? reconnectingState(config.MultiOwnerLightAccount.accountAddress)
                : defaultAccountState(),
            ModularAccountV2: shouldReconnect && config.ModularAccountV2?.accountAddress
