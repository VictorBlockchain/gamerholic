...
export const AuthCard = (props) => {
    return _jsx(AuthCardContent, { ...props });
};
// this isn't used externally
// eslint-disable-next-line jsdoc/require-jsdoc
export const AuthCardContent = ({ className, showClose = false, }) => {
    ...
    const { onAuthSuccess, addPasskeyOnSignup } = useAuthConfig(({ onAuthSuccess, addPasskeyOnSignup }) => ({
        onAuthSuccess,
        addPasskeyOnSignup,
    }));
    ...
    return (_jsx("div", { className: "flex flex-col relative", children: _jsxs(DynamicHeight, { children: [(canGoBack || showClose) && (_jsx(Navigation, { showClose: showClose, showBack: canGoBack, onBack: onBack, onClose: onClose })), _jsx("div", { className: `max-h-[60vh] overflow-auto relative flex flex-col items-center gap-4 text-fg-primary px-6 py-4 ${className ?? ""}`, children: _jsx(Step, {}) }), _jsx(Footer, { authStep: authStep })] }) }));
};
const DynamicHeight = ({ children }) => {
    const contentRef = useRef(null);
    const { height } = useElementHeight(contentRef);
    return (_jsx("div", { className: "transition-[flex-basis] duration-200 ease-out overflow-y-hidden radius-2", style: {
            flexBasis: height ? `${height}px` : undefined,
        }, children: _jsx("div", { className: "z-[1]", ref: contentRef, children: children }) }));
};
