 ...
 */
export const createConfig = (props, ui) => {
    if (ui?.auth?.sections.some((x) => x.some((y) => y.type === "social" && y.mode === "popup"))) {
        props.enablePopupOauth = true;
    }
    const externalWalletSection = ui?.auth?.sections
        .find((x) => x.some((y) => y.type === "external_wallets"))
        ?.find((x) => x.type === "external_wallets");
    if (externalWalletSection?.walletConnect &&
        ...
        props.connectors ?? (props.connectors = []);
