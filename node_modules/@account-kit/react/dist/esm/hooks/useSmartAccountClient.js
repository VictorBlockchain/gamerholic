...
import { getSmartAccountClient, watchSmartAccountClient, } from "@account-kit/core";
 ...
 * ```
 */
export function useSmartAccountClient({ accountParams, type = "ModularAccountV2", ...clientParams }) {
    const { config: { _internal: { wagmiConfig }, }, config, } = useAlchemyAccountContext();
    const result = useSyncExternalStore(watchSmartAccountClient({ type, accountParams, ...clientParams }, config), () => getSmartAccountClient({ type, accountParams, ...clientParams }, config), () => getSmartAccountClient({ type, accountParams, ...clientParams }, config));
