...
const stakingAccounts_1 = require("./stakingAccounts");
const baseWorker_1 = require("../baseWorker");
function nonNullable(value) {
    return value !== null && value !== undefined;
}
const getAllSignatures = async (api, descriptor, fullHistory = true) => {
    let lastSignature;
...
const pushTransaction = async (request) => {
    ...
    if (message.lifetimeConstraint === undefined) {
        const { value: { blockhash, lastValidBlockHeight }, } = await api.rpc.getLatestBlockhash({ commitment: 'finalized' }).send();
    ...
    try {
        ...
        if ((0, web3_js_1.isSolanaError)(error, web3_js_1.SOLANA_ERROR__JSON_RPC__SERVER_ERROR_SEND_TRANSACTION_PREFLIGHT_FAILURE) &&
            (0, web3_js_1.isSolanaError)(error.cause, web3_js_1.SOLANA_ERROR__TRANSACTION_ERROR__BLOCKHASH_NOT_FOUND)) {
...
const getAccountInfo = async (request, isTestnet) => {
    ...
    const getAllTxIds = async (tokenAccountPubkeys) => {
        ...
        const allTxIds = details === 'basic' || details === 'txs' || details === 'txids'
            ? Array.from(new Set((await Promise.all(allAccounts.map(account => getAllSignatures(api, account, details !== 'basic'))))
                .flat()
    ...
    const getTokenAccountsForProgram = (programPublicKey) => api.rpc
        .getTokenAccountsByOwner(publicKey, { programId: (0, web3_js_1.address)(programPublicKey) }, {
        encoding: 'jsonParsed',
    })
        .send();
    ...
    if (details !== 'basic') {
        ...
        if (accountInfo) {
            ...
            const solStakingAccounts = await (0, stakingAccounts_1.getSolanaStakingData)(payload.descriptor, isTestnet, solEpoch, api.clusterUrl);
            misc = {
...
const getAccountProgramSize = (programName) => ({
    staking: wallet_sdk_solana_1.STAKE_ACCOUNT_V2_SIZE,
...
const estimateFee = async (request) => {
    ...
    const priorityFee = await (0, fee_1.getPriorityFee)(api.rpc, message, transaction.signatures);
...
const subscribeBlock = async ({ state, connect, post }) => {
    ...
    const fetchBlock = async () => {
        const { value: { blockhash: blockHash, lastValidBlockHeight: blockHeight }, } = await api.rpc.getLatestBlockhash({ commitment: 'finalized' }).send();
        if (blockHeight) {
            post({
                id: -1,
...
const extractTokenAccounts = (accounts) => accounts
    .map(account => (account.tokens?.map(token => token.accounts?.map(tokenAccount => ({
