...
contract Challenge is ReentrancyGuard, Ownable {
    enum ChallengeType { HEADS_UP, TOURNAMENT }
    // Status codes mapping:
    // 0 CANCELLED, 1 ACTIVE, 2 ACCEPTED, 3 SCORE_REPORTED, 4 SCORE_CONFIRMED, 5 DISPUTED
    enum ChallengeStatus { CANCELLED, ACTIVE, ACCEPTED, SCORE_REPORTED, SCORE_CONFIRMED, DISPUTED }
    ...
    function joinChallenge() external payable challengeActive nonReentrant {
        ...
        if(challengeInfo.payToken == address(0)){
            ...
            require(IERC20(challengeInfo.payToken).balanceOf(participant) >= challengeInfo.entryFee, "Not enough tokens");
            IERC20(challengeInfo.payToken).transferFrom(participant, address(this), challengeInfo.entryFee);
        ...
        if(challengeInfo.tournament != address(0)){
            IBridge(challengeInfo.tournament).setChallenge(address(this), challengeInfo.creator, challengeInfo.opponent);
        }
    ...
    function _checkAndDeclareWinner() internal {
        ...
        if (participantList.length == 2) {
            ...
            if (participants[player1].scoreConfirmed && participants[player2].scoreConfirmed) {
                address gameWinner = participants[player1].score > participants[player2].score ? player1 : player2;
    ...
    function _distributeFunds() internal {
        ...
        if (challengeInfo.payToken == address(0)) {
            ...
            if (platformFeeAmount > 0) {
                require(IERC20(challengeInfo.payToken).transfer(feeRecipient, platformFeeAmount), "Fee transfer failed");
            }
            require(IERC20(challengeInfo.payToken).transfer(winner, prizeAmount), "Prize transfer failed");
        }
        ...
        for (uint256 i = 0; i < participantList.length; i++) {
            ...
            if (participants[participant].hasPaid) {
                if (challengeInfo.payToken == address(0)) {
                    ...
                    require(IERC20(challengeInfo.payToken).transfer(participant, challengeInfo.entryFee), "Refund failed");
                }
            }
            // cleanup in factory
